...
### $FFFF + X Wraparound
  1: Reading from address $FFFF + X, (Where X = 1) loops around to address $0000.  
  2: Reading from address $FFFF + Y, (Where Y = 1) loops around to address $0000.  
  3: Writing to address $FFFF + X (Where X = 1) loops around to address $0000.  
  4: Branching from page $FF to the zero page, and then branching back works.  
  5. Executing address $FFFF will read address $0000 and $0001 as the operands.  

### PPU Register Mirroring
  1: PPU Registers are mirrored through $3FFF  
...
### PPU Register Open Bus
  ...
  2: All PPU Registers update the PPU Data Bus when written.  
  3: Bits 0 through 4 when reading from address $2002 will read the PPU Data Bus.  
  4: The PPU Data Bus value decays.  

### Dummy read cycles
  1: A mirror of PPU_STATUS ($2002) will be read twice by LDA $20F2, X (where X = $10).  
  2: The dummy read does not occur if a page boundary is not crossed.  
  3: The dummy read is on the correct address.  
  4: The STA, X instruction has a dummy read.  
  5: The STA, X dummy read is on the correct address.  
  6: LDA (Indirect), Y does not have a dummy read if a page boundary is not crossed by the Y indexing.  
  7: LDA (Indirect), Y does have a dummy read if a page boundary is crossed by the Y indexing.  
  8: STA (Indirect), Y does not have a dummy read if a page boundary is not crossed by the Y indexing.  
  9: STA (Indirect), Y does have a dummy read if a page boundary is crossed by the Y indexing.  
  A: LDA (Indirect, X) does not have a dummy read.  
  B: STA (Indirect, X) does not have a dummy read.  

### Dummy write cycles
  1: PPU Open Bus should exist.  
  2: Read-Modify-Write instructions write to $2006 twice.  
  3: Read-Modify-Write instructions with X indexing write to $2006 twice.  
...
### Open Bus
  ...
  3: Indexed addressing crossing a page boundary does not update the data bus.  
  4: The upper 3 bits when reading from the controller are open bus.  
  5: Moving the program counter to open bus will read instructions from the floating data bus values.  
  6: Dummy Reads update the data bus.  
  7: Reading from $4015 does not update the databus.  
  8: Writing always updates the databus, even writing to $4015.  

### Unofficial Instructions
  1: NOP Absolute is not a 1-byte NOP.  
  2: NOP Absolute is not a 2-byte NOP.  
  3: NOP Absolute reading address $2002 will clear the VBlank flag.  
...
### Interrupt flag latency
  ...
  3: An IRQ can occur 1 cycle after the final cycle of an SEI instruction. (The SEI instruction polls for interrupts before cycle 2.)  
  ...
  6: The IRQ occurs 1 cycle after the final cycle of an RTI instruction.  (The I flag is pulled off the stack before RTI polls for interrupts.)  
  7: The IRQ should occur 2 instructions after the PLP instruction. (The PLP instruction polls for interrupts before cycle 2.)  
  8: The DMA triggers an IRQ on the correct CPU cycle.  
  9: Branch instructions poll for interrupts before cycle 2.  
  A: Branch instructions do not poll for interrupts before cycle 3.  
  B: Branch instructions poll for interrupts before cycle 4.  
...
### APU Length Counter
  ...
  3: Writing $80 to $4017 will immediately clock the Length Counter.  
  4: Writing $00 to $4017 will not clock the Length Counter.  
  5: Disabling the audio channel will immediately clear the length counter to zero.  
  6: The length counter cannot be set when the channel is disabled.  
  7: If the channel is set to play infinitely, it won't clock the length counter.  
  8: If the channel is set to play infinitely, the length counter is left unchanged.  
...
### Frame Counter IRQ
  1: The IRQ flag is set when the APU Frame counter is in the 4-step mode, and the IRQ flag is enabled.  
  2: The IRQ flag is not set when the APU Frame counter is in the 4-step mode, and the IRQ flag is disabled.  
  3: The IRQ flag is not set when the APU Frame counter is in the 5-step mode, and the IRQ flag is enabled.  
  4: The IRQ flag is not set when the APU Frame counter is in the 5-step mode, and the IRQ flag is disabled.  
  5: Reading the IRQ flag clears the IRQ flag.  
  6: Changing the Frame Counter to 5-step mode after the flag was set does not clear the flag.  
  7: Disabling the IRQ flag will clear the IRQ flag.  
  ...
  C: Reading $4015 on the same cycle the IRQ flag is set, will not clear the IRQ flag. (it gets set again on the following 2 CPU cycles)  
...
### Delta Modulation Channel
  ...
  5: Writing $00 to $4015 will immediately stop the sample.  
  6: Writing to $4013 doesn't change the sample length of the currently playing sample.  
  7: The DMC IRQ Flag should not be set when disabled.  
  8: The DMC IRQ Flag should be set when enabled, and a sample ends.  
  9: Reading $4015 does not clear the IRQ flag.  
  A: Writing to $4015 does clear the IRQ flag.  
  B: Disabling the IRQ flag clears the IRQ flag.  
  ...
  F: Clearing the looping flag will not immediately end the sample. The sample will then play for it's remaining bytes.  
  G: A looping sample will re-load the sample length from $4013 every time the sample loops.  
...
### DMA + $4016 Read
  1: The DMC DMA was either on the wrong cycle, or the halt/alignment cycles did not read from $4016, clocking the controller port.  
...
### APU Register Activation
  ...
  2: Reading from $4015 did not clear the APU Frame Counter Interrupt flag.  
...
### Instruction Timing
  1: The NMI timing is not relaible enough.  
  2: LDA Immediate takes 2 cycles.  
  3: LDA Zero Page tales 3 cycles.  
  4: LDA Absolute takes 4 cycles.  
  5: LDA Absolute, X takes 4 cycles if a page boundary is not crossed.  
  6: LDA Absolute, X takes 5 cycles if a page boundary is crossed.  
  7: LDA (Indirect, Y) takes 5 cycles in a page boundary is not crossed.  
  8: LDA (Indirect, Y) takes 6 cycles in a page boundary is crossed.  
  9: JMP Absolute takes 3 cycles  
  A: LDA (Indirect, X) takes 6 cycles  
  B: ASL A, ROL A, LSR A, ROR A, TSX, TXS, TAX, TXA, TAY, TYA, DEX, INX, DEY, INY, and NOP all take 2 cycles.  
  C: JSR takes 6 cycles  
  D: RTS takes 6 cycles  
  E: PHA takes 3 cycles  
  F: PLA takes 4 cycles  
  G: PHP takes 3 cycles  
  H: PLP takes 4 cycles  
...
### Sprite 0 Hit behavior
  ...
  6: Sprite zero hits can happen at X=254.  
  7: Sprite zero hits cannot happen at X=255.  
  ...
  A: Despite the 8 pixel mask, if the sprite has visible pixels beyond the mask the Sprite Zero Hit occurs.  
  B: Sprite zero hits can happen at Y=238.  
  C: Sprite zero hits cannot happen at Y>=239  
  ...
  E: The sprite zero hit flag is not set too early.  
...
### Sprite overflow behavior
  ...
  2: The Sprite Overflow Flag is not the same thing as the CPU's Overflow flag.  
...
### RMW $2007 Extra Write
  ...
  4: If "v" is pointing to Palette RAM, this extra write simply occured at "v" after it was incremented from the read cycle.  
